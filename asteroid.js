...
function anime60fps() {
    ...
    for (var i = 0; i < AstArray.length; i++) {
        var meteore = AstArray[i];

        // 1) On bouge les meteores
        meteore.move();

        // 2) collision test with walls
        collisionTestWithWalls(meteore);

        // collision test with bullets
        collisionTestAsteroidBullets(meteore, Vaisseau1.bullets);
        //collision tets  asteroid et vaisseau
        collisionTestAsteroidVaisseau(meteore, Vaisseau1);

        collisionTestVaisseauBonus(Vaisseau1, BonusArray);

        // 3) On dessine les meteore
        meteore.draw();
